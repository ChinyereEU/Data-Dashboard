...
const App = () => {
  ...
  useEffect(() => {
    const fetchAllCreatorData = async (retries = 3) => {
      try {
        // const response = await fetch(`https://gateway.marvel.com:443/v1/public/creators?orderBy=lastName&apikey=${API_KEY}`);
        const response = await fetch(`https://gateway.marvel.com:443/v1/public/comics?format=digital%20comic&formatType=comic&noVariants=true&hasDigitalIssue=true&orderBy=title&apikey=${API_KEY}`);
        if (response.ok) {
          const json = await response.json();
          console.log("API Response:", json);
